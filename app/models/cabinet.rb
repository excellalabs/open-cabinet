class Cabinet < ActiveRecord::Base
  ...
  def identify_primary(med_name) # method to call when med is newly selected to become primary
    ...
    medicine = self.medicines.first if medicine.nil?
    rebuild_cabinet
    ...
    medicines.each do |current_med|
      medicines.each do |med|
        ...
        if determine_interaction(current_med, med)
          med.interactions.push med 
          current_med.interactions.push med
        end
      end
    end
  end
